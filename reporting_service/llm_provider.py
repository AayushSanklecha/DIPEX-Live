...
class LLMProvider:
    ...
    def generate_summary(
        ...
        # ── Use InsightNarrator for a rich, structured narrative ──────────
        try:
            from .insight_narrator import NarrativeBuilder
            builder = NarrativeBuilder()
            narrative = builder.build(
                eda_report=verified_result.get("eda_report"),
                model_metrics=verified_result.get("metrics") or verified_result.get("model_metrics"),
                analyst_flags=verified_result.get("analyst_flags"),
                risk_flags=verified_result.get("risk_flags"),
                gate1_decision=gate,
                gate2_decision=gate,
                confidence_score=confidence,
                row_count=int(verified_result.get("row_count", 0)),
                col_count=int(verified_result.get("col_count", 0)),
                run_id=run_id,
            )
            ...
            narrative += (
                "\n\n---\n*Report generated by DIPEX deterministic InsightNarrator. "
                "Set `HF_API_KEY` in `.env` for AI-enhanced narrative.*"
            )
            return narrative
        except Exception as exc:
            logger.warning("InsightNarrator unavailable (%s) — using compact fallback.", exc)

        # ── Compact fallback (InsightNarrator unavailable) ────────────────
        lines = [
            ...
            "Set `HF_API_KEY` in your `.env` file to enable AI-generated narrative.",
            "\n*All results independently verified. No speculation in this summary.*",
        ]
        return "\n\n".join(lines)

    def generate_compliance_remediation(
        self,
        violations: list,
        domain: str,
        run_id: str = "",
        ...
        """
        Base (rule-based fallback) compliance remediation generator.
        Returns static per-rule remediation text when LLM is unavailable.
        """
        if not violations:
            return ""
        ...
        lines = []
        for v in violations:
            rule   = v.get("rule", v.get("rule_name", "unknown"))
            sev    = v.get("severity", "WARNING")
            msg    = v.get("message", "")
            lines.append(f"**[{sev}] {rule}**: {msg}")

        return (
            f"### Compliance Remediation — {domain.upper()} Domain\n\n"
            + "\n\n".join(lines)
...
class HuggingFaceProvider(LLMProvider):
    ...
    def generate_summary(
        ...
        # Governance gate first — super() returns GOVERNANCE BLOCK or rule-based narrative
        fallback = super().generate_summary(verified_result, max_words=max_words, run_id=run_id)
        if "[GOVERNANCE BLOCK]" in fallback:
            return fallback

        # ── Build rich prompt using InsightNarrator pre-computed interpretations ──
        try:
            from .insight_narrator import NarrativeBuilder
            prompt = NarrativeBuilder().build_hf_prompt(
            ...
            logger.warning("InsightNarrator prompt build failed (%s) — using basic prompt.", exc)
            ...
            prompt = (
                ...
                f"1. **Executive Summary** — overall verdict\n"
                f"2. **Analytical Insights** — what patterns mean in business terms\n"
                f"3. **Data Quality** — issues and business impact\n"
                f"4. **Recommended Actions** — 3-5 concrete next steps\n\n"
