...
# ── POST /ingest/analyst/{operation} ─────────────────────────────────────────

@router.post("/analyst/{operation}", summary="Run analyst operation on Gold layer copy")
async def run_analyst_operation(
    operation: str,
    body: Dict[str, Any],
    intake: UniversalIntake = Depends(_get_intake),
):
    """
    Run a junior or senior analyst operation on a Gold copy derived from Silver.

    Body fields::

        dataset_id      : str
        snapshot_id     : str   (ISSF snapshot to derive Gold from)
        level           : 'junior' | 'senior'
        parameters      : dict  (operation-specific params, e.g. query, group_by, target_col)

    Silver is NEVER mutated. All work happens on a Gold copy.
    Returns the Gold artefact metadata + lineage.
    """
    try:
        import pandas as pd
        ...
        # Load Silver from snapshot parquet or ISSF data
        snap_dir = getattr(intake, "snapshot_dir", "data/snapshots")
        parquet_path = f"{snap_dir}/{snapshot_id}_issf.parquet"
        if not __import__("os").path.exists(parquet_path):
            _err(404, f"Silver parquet for snapshot '{snapshot_id}' not found")

        df = pd.read_parquet(parquet_path)
        lm = LayerManager()
        silver = ImmutableDataFrame(df, layer="silver", dataset_id=dataset_id)
        jr = JuniorAnalyst(layer_manager=lm, operator="api")
        sr = SeniorAnalyst(layer_manager=lm, operator="api")

        op_map_junior = {
            "basic_cleaning":        lambda s: jr.basic_cleaning(s, source_snapshot_id=snapshot_id),
            "remove_duplicates":     lambda s: jr.remove_duplicates(s, source_snapshot_id=snapshot_id),
            "filter_rows":           lambda s: jr.filter_rows(s, query=params.get("query", "index >= 0"),
                                                               source_snapshot_id=snapshot_id),
            "simple_aggregation":    lambda s: jr.simple_aggregation(
                s, group_by=params.get("group_by", [df.columns[0]]),
                agg_map=params.get("agg_map", {}), source_snapshot_id=snapshot_id),
            "kpi_tracking":          lambda s: jr.kpi_tracking(s, kpi_definitions=params.get("kpis", {}),
                                                                source_snapshot_id=snapshot_id),
            "percent_change":        lambda s: jr.percent_change(s, col=params.get("col", df.columns[0]),
                                                                  source_snapshot_id=snapshot_id),
            "manual_threshold_check": lambda s: jr.manual_threshold_check(
                s, thresholds=params.get("thresholds", {}), source_snapshot_id=snapshot_id),
        }
        op_map_senior = {
            "feature_engineering":   lambda s: sr.feature_engineering(
                s, numeric_cols=df.select_dtypes("number").columns.tolist(),
                source_snapshot_id=snapshot_id),
            "risk_scoring":          lambda s: sr.risk_scoring(
                s, risk_signals=params.get("risk_signals", {}), source_snapshot_id=snapshot_id),
            "windowed_aggregation":  lambda s: sr.windowed_aggregation(
                s, value_col=params.get("value_col", df.select_dtypes("number").columns[0]),
                source_snapshot_id=snapshot_id),
            "executive_summary":     lambda s: sr.executive_summary(
                s, target_col=params.get("target_col"), source_snapshot_id=snapshot_id),
            "drift_analysis":        lambda s: sr.drift_analysis(
                s, baseline_snapshot_id=params.get("baseline_snapshot_id", ""),
                source_snapshot_id=snapshot_id),
            "scenario_simulation":   lambda s: sr.scenario_simulation(
                s, scenarios=params.get("scenarios", {}),
                target_col=params.get("target_col", df.columns[0]),
                source_snapshot_id=snapshot_id),
        }

        op_map = {**op_map_junior, **op_map_senior}
        if operation not in op_map:
            _err(400, f"Unknown operation '{operation}'", f"Available: {list(op_map)}")

        gold = op_map[operation](silver)
        return {
            "status": "ok",
            "operation": operation,
            "gold_dataset_id": gold.dataset_id,
            "gold_shape": list(gold.data.shape),
            "gold_checksum": gold.checksum[:24],
            "lineage_id": gold.lineage.lineage_id,
            "source_snapshot_id": snapshot_id,
            "silver_unchanged": True,
            "transformation_steps": len(gold.lineage.transformation_steps),
        }
    except HTTPException:
        raise
    except Exception as exc:  # noqa: BLE001
        logger.exception("Analyst operation '%s' failed", operation)
        _err(500, f"Analyst operation '{operation}' failed", str(exc))
